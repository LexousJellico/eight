import {
  ...
  export function adminBasePath(role: AdminResourceRole, section: string): string {
  ...
  export function statusBadgeTone(value?: unknown): string {
    ...
    if (['confirmed', 'approved', 'active', 'completed', 'paid', 'resolved', 'closed'].includes(normalized)) {
      return 'border-emerald-500/25 bg-emerald-500/10 text-emerald-700 dark:text-emerald-200';
    }

    if (['pending', 'partial', 'for_review', 'new', 'open', 'unread'].includes(normalized)) {
